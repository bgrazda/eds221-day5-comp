...
# The line below creates a time vector of 8 years
time_yr <- seq(from = 0, to = 8, by = 1)


# The line below creates an empty matrix to return the population at each life stage at each year
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below storing the initial population in the first row
proj_year[1, ] <- fish_year0

# The for loop below storing the population at each year following the initial population row by row using dot product (using matrix and multiplying the previous proj year)
